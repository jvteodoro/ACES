module fftAddressGenerator
  ...
  (
   ...
   localparam MEM_ADDR_W = FFT_N-1;
   localparam [MEM_ADDR_W-1:0] MEM_ADDR_ONE = {{(MEM_ADDR_W-1){1'b0}},1'b1};
   reg [MEM_ADDR_W-1:0] runCount;
   wire [MEM_ADDR_W-1:0] runCount_pp = runCount + MEM_ADDR_ONE;
   ...
   reg [MEM_ADDR_W-1:0] twiddleFactorCounter;
   ...
   reg [MEM_ADDR_W-1:0] memAddr_w;
   assign MemAddr = memAddr_w;

   reg [MEM_ADDR_W-1:0]   memAddrLowerMask_;
   always_comb begin
      if ( (stageCount == 0) ||
	   (stageCount == 1) ) begin
	 memAddrLowerMask_ = 0;
      end else begin
         memAddrLowerMask_ = (MEM_ADDR_ONE << (stageCount-1)) - MEM_ADDR_ONE;
      end
   end
   
   reg [MEM_ADDR_W-1:0] runCountLsbShift_;
   always_comb begin
      if ( (stageCount == 0) ||
	   (stageCount == 1) ) begin
	 runCountLsbShift_ = 0;
      end else begin
         runCountLsbShift_ = ( {{(MEM_ADDR_W-1){1'b0}},runCount[0]} << (stageCount-1) );
      end
   end

   reg [MEM_ADDR_W-1:0] memAddrHigherMask_;
   always_comb begin
      if ( (stageCount == 0) ||
	   (stageCount == 1) ) begin
         memAddrHigherMask_ = {MEM_ADDR_W{1'b1}};
      end else begin
         memAddrHigherMask_ = ~((MEM_ADDR_ONE << stageCount) - MEM_ADDR_ONE);
      end
   end   
   always_comb begin
      case ( stageCount )
	...
	default:
	  begin
	     memAddr_w = 
                 (memAddrLowerMask_  & {1'b0,runCount[MEM_ADDR_W-1:1]} )|
			 (runCountLsbShift_)|
                 (memAddrHigherMask_ & runCount[MEM_ADDR_W-1:0]);
	  end
      endcase
   end
