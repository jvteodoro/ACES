...
RUN_DIR="${REPO_ROOT}/sim/local/modelsim/gui_${TOP_MODULE}"
mkdir -p "${RUN_DIR}"
...
if command -v vsim >/dev/null 2>&1; then
  ...
  echo "Info: ferramentas ModelSim GUI nao estao no PATH do Linux; encaminhando para open_modelsim_gui.ps1 via powershell.exe." >&2
  aces_run_repo_powershell_script "${REPO_ROOT}" "sim/manifest/scripts/open_modelsim_gui.ps1" "${FILELIST}" "${TOP_MODULE}"
else
