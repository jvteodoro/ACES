module top_level_i2s_fft_tx_sine_1k #(
...
`include "top_level_i2s_fft_tx_sine_1k_lut.svh"
    ...
    assign diag_fft_real_i  = sine_1k_fft_real_lut(diag_bin_index_r);
    assign diag_fft_imag_i  = sine_1k_fft_imag_lut(diag_bin_index_r);
    assign diag_fft_last_i  = (diag_bin_index_r == BIN_IDX_W'(DIAG_WINDOW_BINS-1));
    assign diag_bfpexp_i    = SINE_1K_BFPEXP_C;
    ...
    initial begin
        if (FFT_DW != 18)
            $error("top_level_i2s_fft_tx_sine_1k: FFT_DW deve permanecer em 18 para casar com o contrato do host.");
        if (DIAG_WINDOW_BINS != SINE_1K_FRAME_BINS)
            $error("top_level_i2s_fft_tx_sine_1k: DIAG_WINDOW_BINS deve casar com o LUT gerado.");
        if (DIAG_WINDOW_BINS > 512)
            $error("top_level_i2s_fft_tx_sine_1k: DIAG_WINDOW_BINS deve caber na faixa 512..1023 do protocolo I2S.");
    end
