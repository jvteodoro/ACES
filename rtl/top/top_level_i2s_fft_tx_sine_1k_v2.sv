module top_level_i2s_fft_tx_sine_1k_v2 #(
    ...
    parameter int DIAG_BFPEXP_HOLD_FRAMES   = 128,
    parameter int DIAG_TEST_BFPEXP          = 15
)(
    ...
    localparam logic signed [BFPEXP_W-1:0] DIAG_TEST_BFPEXP_C = DIAG_TEST_BFPEXP;

`include "top_level_i2s_fft_tx_sine_1k_lut.svh"
    ...
    // Pre-scale the raw bins so host-side BFPEXP compensation should recover
    // essentially the same spectrum as the BFPEXP=0 reference top.
    function automatic logic signed [FFT_DW-1:0] apply_test_bfpexp_pre_scale(
        input logic signed [FFT_DW-1:0] lut_value_i
    );
        begin
            if (DIAG_TEST_BFPEXP <= 0)
                apply_test_bfpexp_pre_scale = lut_value_i;
            else
                apply_test_bfpexp_pre_scale = lut_value_i >>> DIAG_TEST_BFPEXP;
        end
    endfunction
    ...
    assign diag_fft_real_i  = apply_test_bfpexp_pre_scale(sine_1k_fft_real_lut(diag_bin_index_r));
    assign diag_fft_imag_i  = apply_test_bfpexp_pre_scale(sine_1k_fft_imag_lut(diag_bin_index_r));
    assign diag_fft_last_i  = (diag_bin_index_r == BIN_IDX_W'(DIAG_WINDOW_BINS-1));
    assign diag_bfpexp_i    = DIAG_TEST_BFPEXP_C;

    assign diag_accept_w      = diag_fft_valid_i && diag_fft_ready_o;
    assign diag_window_done_w = diag_accept_w && diag_fft_last_i;
    assign unused_inputs_probe = 1'b0;

    initial begin
        if (FFT_DW != 18)
            $error("top_level_i2s_fft_tx_sine_1k_v2: FFT_DW deve permanecer em 18 para casar com o contrato do host.");
        if (DIAG_WINDOW_BINS != SINE_1K_FRAME_BINS)
            $error("top_level_i2s_fft_tx_sine_1k_v2: DIAG_WINDOW_BINS deve casar com o LUT gerado.");
        if (DIAG_WINDOW_BINS > 512)
            $error("top_level_i2s_fft_tx_sine_1k_v2: DIAG_WINDOW_BINS deve caber na faixa 512..1023 do protocolo I2S.");
        if (DIAG_TEST_BFPEXP < 0)
            $error("top_level_i2s_fft_tx_sine_1k_v2: DIAG_TEST_BFPEXP deve ser >= 0.");
        if (DIAG_TEST_BFPEXP >= FFT_DW)
            $error("top_level_i2s_fft_tx_sine_1k_v2: DIAG_TEST_BFPEXP deve ser < FFT_DW para preservar os bins reescalados.");
    end
    ...
    always_comb begin
        ...
        unique case ({sw1, sw0})
            2'b00: dbg_word = {{(24-BFPEXP_W){diag_bfpexp_i[BFPEXP_W-1]}}, diag_bfpexp_i};
            2'b01: dbg_word = {{(24-FFT_DW){diag_fft_real_i[FFT_DW-1]}}, diag_fft_real_i};
            2'b10: dbg_word = {{(24-FFT_DW){diag_fft_imag_i[FFT_DW-1]}}, diag_fft_imag_i};
            2'b11: begin
    ...
    i2s_fft_tx_adapter #(
        ...
        .FIFO_DEPTH(DIAG_WINDOW_BINS + DIAG_BFPEXP_HOLD_FRAMES + 4),
    ...
    assign gpio_0_d27 = 1'b0;
    assign gpio_0_d28 = 1'b0;
    assign gpio_0_d29 = 1'b0;
    assign gpio_0_d30 = tx_i2s_sck_o;
    assign gpio_0_d31 = 1'b0;
